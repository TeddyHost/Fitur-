...
bot.command('tiktok', async (ctx) => {
  ...
  if (!url || !url.startsWith('http')) return ctx.reply('❌ Masukkan link TikTok yang valid.');

  await ctx.reply('⏳ Mengambil video...');

  try {
    const result = await tiktokDl(url);
    if (!result.status) return ctx.reply('❌ Gagal mengambil data.');
    ...
    if (result.durations === 0) {
      ...
      const video = result.data.find(v => v.type === 'nowatermark_hd' || v.type === 'nowatermark');
